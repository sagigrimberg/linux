...
__frwr_recovery_worker(struct work_struct *work)
{
	...
	if (r->r.frmr.sg_gaps)
		mr_type = IB_MR_TYPE_SG_GAPS_REG;
	else
		mr_type = IB_MR_TYPE_MEM_REG;

	r->r.frmr.fr_mr = ib_alloc_mr(pd, mr_type, depth);
...
__frwr_init(struct rpcrdma_mw *r, struct ib_pd *pd, struct ib_device *device,
	    unsigned int depth, bool sg_gaps)
{
	...
	f->fr_mr = ib_alloc_mr(pd, mr_type, depth);
	if (IS_ERR(f->fr_mr))
		goto out_mr_err;
...
frwr_op_init(struct rpcrdma_xprt *r_xprt)
{
	...
	if (!rc && devattr.device_cap_flags_ex & IB_DEVICE_SG_GAPS_REG)
		sg_gaps = true;
	...
	while (i--) {
		...
		rc = __frwr_init(r, pd, device, depth, sg_gaps);
		if (rc) {
			kfree(r);
			return rc;
		}
	    ...
	    int nsegs, bool writing)
{
	...
	n = ib_map_mr_sg(mr, frmr->sg, frmr->sg_nents);
	...
	ib_set_reg_wr(mr, mr->rkey, access, false, &reg_wr);
